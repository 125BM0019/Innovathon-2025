...
async function initializeAndStartServer() {
  ...
  try {
    ...
    // --- 4. ROUTES (Start of Express Logic) ---
    ...
    app.post ('/api/register', async (req, res) => {
        // FIX: Destructure 'username' to match database column
        ...
        try {
            ...
            // 3. Create a new user (using 'username')
            const newUser = await pool.query(
                // FIX: Insert into 'username' column
    ...
    app.post('/api/login', async (req, res) => {
        ...
        try {
            ...
            const isMatch = await bcrypt.compare(password, user.password_hash); // CRITICAL FIX: Use bcrypt comparison

            if (isMatch) {
                // CRITICAL CHANGE: Store the logged-in user's ID
                currentLoggedInUser = user.id; // Use the 'id' from the database
    ...
    app.get('/api/user', async (req, res) => {
      ...
      try {
        // FIX: Select based on 'id' column, not 'user_id'
        const result = await pool.query('SELECT id, username, email FROM users WHERE id = $1', [currentLoggedInUser]); 
